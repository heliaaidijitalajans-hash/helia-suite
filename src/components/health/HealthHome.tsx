...
import { workouts, type Workout } from "@/data/health-workouts";
...
function RecommendedSection({
  ...
  return (
    ...
    >
      <div className="relative overflow-hidden rounded-[1.4rem] border border-zinc-200/90 bg-gradient-to-br from-white via-zinc-50/80 to-emerald-50/40 p-[1px] shadow-[0_24px_60px_-32px_rgba(15,23,42,0.18),0_0_0_1px_rgba(255,255,255,0.8)_inset]">
        <div
          aria-hidden
          className="pointer-events-none absolute -left-20 top-0 h-40 w-40 rounded-full bg-emerald-400/[0.12] blur-3xl"
        />
        <div
          aria-hidden
          className="pointer-events-none absolute -right-16 bottom-0 h-36 w-36 rounded-full bg-cyan-400/[0.09] blur-3xl"
        />
          ...
          <div className="space-y-6">
            <div className="flex flex-wrap items-center gap-2 sm:gap-2.5">
              <span className="inline-flex items-center gap-1.5 rounded-full bg-gradient-to-r from-zinc-900 to-zinc-800 px-3 py-1.5 text-[10px] font-semibold uppercase tracking-[0.16em] text-white shadow-sm ring-1 ring-white/15">
                <Sparkles className="h-3.5 w-3.5 shrink-0 text-emerald-200" strokeWidth={2} aria-hidden />
                Helia AI
              </span>
              <span className="rounded-full border border-emerald-200/80 bg-emerald-50/95 px-3 py-1.5 text-[10px] font-semibold uppercase tracking-[0.12em] text-emerald-800 shadow-[0_6px_20px_-10px_rgba(16,185,129,0.35)]">
                For you
              </span>
            </div>
            <div className="space-y-2.5">
              ...
              <p className="max-w-[22rem] text-[13px] leading-[1.55] text-zinc-500 sm:text-sm sm:leading-relaxed">
                Personalized by Helia AI — static demo data, no backend.
              </p>
...
export function HealthHome() {
  ...
  const todaysPlan = useMemo(() => workouts.filter((w) => w.isTodaysPlan), []);
  const recommended = useMemo(() => workouts.filter((w) => w.isRecommended).slice(0, 8), []);

  const railBeforeAll = todaysPlan.length + recommended.length;
    ...
    <div className="flex min-h-0 flex-1 flex-col">
      ...
      >
        <div className="flex items-start justify-between gap-3">
          <div>
            ...
            <p className="mt-2 max-w-[18rem] text-[13px] leading-relaxed text-zinc-500">
              AI-powered fitness & wellness system
            </p>
          </div>
          <Link
      ...
      >
        <motion.div
          ...
          transition={reduce ? { duration: 0 } : { type: "spring", stiffness: 260, damping: 30, delay: 0.06 }}
        ...
        >
          <div className="relative overflow-hidden rounded-[15px] px-5 py-6">
            <div aria-hidden className="pointer-events-none absolute -right-16 -top-20 h-48 w-48 rounded-full bg-white/[0.07] blur-3xl" />
            <div aria-hidden className="pointer-events-none absolute -bottom-24 left-0 h-56 w-56 rounded-full bg-emerald-400/[0.09] blur-3xl" />
            ...
            <p className="relative mt-3 max-w-[17rem] text-lg font-semibold leading-snug tracking-tight text-white">
              Smart sessions that fit your week
            </p>
            <p className="relative mt-3 max-w-[18rem] text-[13px] leading-relaxed text-zinc-400">
              Recommended workouts, a daily plan, and AI insights — static demo, no backend.
            </p>
      ...
      <RecommendedSection items={recommended} motionBase={0} delay={0.06} reduce={reduce} />

      <WorkoutRailSection
        title="Today’s Plan"
        subtitle="A short sequence tuned for today (demo)"
        items={todaysPlan}
        motionBase={recommended.length}
        delay={0.1}
        reduce={reduce}
      />
      ...
      >
        <div className="space-y-1">
          <h2 className="text-[0.9375rem] font-semibold tracking-tight text-zinc-900">All workouts</h2>
          <p className="text-[12px] text-zinc-500">Browse the full demo catalog</p>
        </div>
        <div className="grid grid-cols-2 gap-4">
          {workouts.map((workout, index) => (
            <WorkoutCard key={workout.id} workout={workout} motionIndex={index + railBeforeAll} />
          ))}
        </div>
      </motion.section>
    </div>
