...
import { getHealthCategoryById, getWorkoutsByCategoryId } from "@/data/health-workouts";
...
export function HealthCategoryDetailView({ categoryId }: { categoryId: string }) {
  const router = useRouter();
  const reduce = useReducedMotionHydrationSafe();
  const basePath = useMarketBasePath();
  const category = getHealthCategoryById(categoryId);
  const catalog = getWorkoutsByCategoryId(categoryId);
    ...
    <div className="pb-8">
      <div className="relative px-6 pb-5 pt-3">
        ...
        >
          ...
          <div className="mt-3 flex items-center justify-center gap-1.5 text-emerald-600">
            ...
            <span className="text-[12px] font-medium text-zinc-400">workouts</span>
          </div>
        </motion.div>
      </div>
      ...
      <div className="px-6">
        <div className="mb-4 flex items-end justify-between gap-3">
          <div>
            <h2 className="text-[0.9375rem] font-semibold tracking-tight text-zinc-900">Workouts</h2>
            <p className="text-[12px] text-zinc-500">{catalog.length} in this category</p>
          </div>
        </div>
        <div className="grid grid-cols-2 gap-4">
          {catalog.map((workout, index) => (
            <WorkoutCard key={workout.id} workout={workout} motionIndex={index} />
          ))}
        </div>
      </div>
    </div>
