...
/** @type {any[]} */
const routes = [];
/** @type {{ method: string, path: string, authentication: string, category: string }[]} */
const endpoints = [];
...
/** Public copy — method/path/auth/category only (no source paths or debug). */
...
if (endpoints.length === 0) {
  console.error(
    "ERROR: Generated 0 endpoints. Expected App Router at src/app/api."
  );
  process.exit(1);
}

console.error(
  `[api-manifest] Wrote ${endpoints.length} endpoints → server + public (sanitized)`
);
